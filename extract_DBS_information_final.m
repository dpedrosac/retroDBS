%% Extract DBS-Settings from PDFs 
% for some particular cases, this script is not able to perform extraction. These include: 
% '601476' single electrode in Program 2
% '735020', '736106' both files have four electrodes registered in one program
% '736277' single electrode in left VIM
% '736497' single electrode in Program
% '736632' single electrode and four electrodes in different programs
% 736805 CASE is not a pole
% 737323 BSC DB-2201 
% 737817 BSC DB-2201 
% 738640 left Gpi only
% 738730 BSC DB-2201 
% Cave: 739190 and 738753 have one file with 8 electrode contacts each
% 740710 BSC DB-2201 
% 740713 all January pdfs have programs w/ 4 electrodes per program
% 741164 BSC DB-2201 
...
path = '..\Retro_DBS\imagingDBS2021\protocols\';
cd(path);
dirData = dir;      % Get *.nii s for the current directory
folders = {dirData.name}';  % Get a list of the folders
index = contains(folders,'.') | contains(folders,{'735020', '736106', '736277', '736497', '736632', '736805', ...
    '737323', '737817', '738640', '738730', '740710', '741164'});
folders(index) = [];


Table_all_Pat_max_use = table;



for i=1:size(folders,1)
    cd (strcat(path, folders{i,1})); % cd to new directory
    ...
    for l=1:size(data,1)
        ...
        if ~contains(str,'0___12___24___Ho_ur_s')
            ...
            % manually check whether Contact 4 or 3 was active. This holds true for most other single active
            % contacts except 1, 8, 9, and 16.
              
            expression = ['___4___2___3_|'... % no contact active   +
                '(?<Contact_4>\d+)%___4___2___3_?|'... %contact 4 active 
                '(?<Contact_4>\d+)%___4___2___3_+|'...
                '(?<Contact_2>\d+)%___4___2___3___?|'... %contact 2 active
                '(?<Contact_2>\d+)%___4___2___3___+|'...
                '(?<Contact_4>\d+)%___(?<Contact_2>\d+)%___4___2___3___?___?|'... % contact 4 and 2 active
                '(?<Contact_4>\d+)%___(?<Contact_2>\d+)%___4___2___3___+___+|'...
                '(?<Contact_2>\d+)%___(?<Contact_3>\d+)%___4___2___3_?___?|'... %contact 2 and 3 active 
                '(?<Contact_2>\d+)%___(?<Contact_3>\d+)%___4___2___3_+___+|'...
                '(?<Contact_4>\d+)% (?<Contact_3>\d+)%___4___2___3___??|'... %contact 4 and 3 active
            ...
            if contains(str,'12___10___11')
                ...
                Contacts = [struct2cell(Contact_1')', struct2cell(Contact_4')', ...
                    ...
                    struct2cell(Contact_9')', struct2cell(Contact_12')', ...
                ...
                Contacts.Properties.VariableNames = {'Contact_1', 'Polarity_1', 'Contact_2', 'Contact_3', ...
                    ...
                    'Contact_9', 'Polarity_9', 'Contact_10', 'Contact_11', 'Contact_12', 'Contact_13', ...
                ...
                Contacts_Left= [struct2cell(Contact_1(1:2:end)')', struct2cell(Contact_4(1:2:end)')', ...
                    struct2cell(Contact_7(1:2:end)')', struct2cell(Contact_8(1:2:end)')'];
                Contacts_Right = [struct2cell(Contact_1(2:2:end)')', struct2cell(Contact_4(2:2:end)')', struct2cell(Contact_7(2:2:end)')', ...
                    struct2cell(Contact_8(2:2:end)')'];
...
writetable(Table_all_Pat_max_use, 'All_Pts_Data_max_usage_blabla.xlsx');
